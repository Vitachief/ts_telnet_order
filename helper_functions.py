...
def connect_and_get_chan_list():
    with Telnet('localhost', 25639) as ts:
        ...
        print_chan_info('channelslist_formatted.txt', chan_info_list)
        print_ts_list('channelslist.txt', chanlist_answ)


def connect_and_print_chan_list():
    with Telnet('localhost', 25639) as ts:
        print("Authenticating...")
        ts.write(b'auth apikey=' + apikey + b'\n')
        auth_answ = ts.read_until(b'neverhappen', 1).decode('utf8')
        ...
        ts.read_very_eager()
        chan_info_list = parse_chan_info(chanlist_answ)
        for info_str in chan_info_list:
            print('{} {}'.format(info_str.name, info_str.cid))


def get_clients(chan_num):
    with Telnet('localhost', 25639) as ts:
        ts.write(b'auth apikey=' + apikey + b'\n')
        auth_answ = ts.read_until(b'neverhappen', 1).decode('utf8')
        ts.write(b'channellist\n')
        ...
        parent_chanlists = get_children_list(chan_info_list, chan_num)
...
def do_work():
    ...
    create_folders({target_folder, debug_folder, target_folder_curr})
    with Telnet('localhost', 25639) as ts:
        print("Authenticating...")
        ts.write(b'auth apikey=' + apikey + b'\n')
        auth_answ = ts.read_until(b'neverhappen', 1).decode('utf8')
